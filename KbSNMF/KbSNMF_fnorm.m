function [A,S,avg_kurt,smooth_fnorm,obj_fun,C,time] = KbSNMF_fnorm(X,A_init,S_init,tmax,Cmin,gamma,theta)

% Author : Mevan Ekanayake (mevanekanayake@gmail.com)
% Title  : Kurtosis based Smooth NMF based on frobenius norm (KbSNMF-fnorm)
...
%           (4) smooth_fnorm = a vector containing values of the smooth frobenius norm between X and estimated AS
...
while iter<tmax && (abs(Lt1-Lt2)/abs(Lt1))>Cmin
    ...
    %update A
    MS = M*S;
    A = max(myeps,A.*(X*MS')./(A*(MS*MS')+gamma_*N*(N*A).^3));
    ...
    smooth_fnorm(iter) = norm(X-A*M*S,'fro')^2;
    obj_fun(iter) = smooth_fnorm(iter)-gamma*avg_kurt(iter);
    ...
    fprintf('iter = %04d, avg_kurt = %.4e, smooth_fnorm = %.4e, obj_fun = %.4e, C = %.4e\n',iter,avg_kurt(iter),smooth_fnorm(iter),obj_fun(iter),C(iter));
